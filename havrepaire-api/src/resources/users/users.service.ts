      ...
      try {
        if (deletedUser.comments) {
          for (const comment of deletedUser.comments) {
            const deletedComment = await this.commentModel.findByIdAndDelete(comment._id).populate('article').exec();
            if (!deletedComment) throw new Error (`Comment with id ${comment._id} was not found`);
            try {
              await this.articleModel.findByIdAndUpdate(deletedComment.article._id, {'$pull': {'comments': deletedComment._id}}).populate('comments').exec();
            } catch (e) {
              throw new Error (`Oups, comment with id ${comment._id}could not be removed from article with id ${deletedComment.article._id}:${e}`);
            }
          }
        }
      } catch (e) {
