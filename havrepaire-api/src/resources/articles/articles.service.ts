      ...
      try {
        if (deletedArticle.comments) {
          for (const comment of deletedArticle.comments) {
            const deletedComment = await this.commentModel.findByIdAndDelete(comment._id).populate('author').exec();
            if (!deletedComment) throw new Error (`Comment with id ${comment._id} was not found`);
            try {
              await this.userModel.findByIdAndUpdate(deletedComment.author._id, {'$pull': {'comments': deletedComment._id}}).populate('comments').exec();
            } catch (e) {
              throw new Error (`Oups, comment with id ${comment._id}could not be removed from user with id ${deletedComment.author._id}:${e}`);
            }
          }
        }
      } catch (e) {
