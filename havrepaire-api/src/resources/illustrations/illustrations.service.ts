...
export class IllustrationsService {
  ...
  async create(createIllustrationDto: CreateIllustrationDto) {
    try {
      ...
      const illustration = new this.illustrationModel(createIllustrationDto);
      const createdIllustration = await illustration.save();
      // update user if avatar
      try {
        createIllustrationDto.userId && await this.userModel.findByIdAndUpdate(new ObjectId(createIllustrationDto.userId), {'avatar': createdIllustration}).exec();
      } catch (e) {
        throw new Error (`User could not be updated during illustration creation: ${e}`)
      }
      // update article if article illustration
      try {
        createIllustrationDto.articleId && await this.articleModel.findByIdAndUpdate(new ObjectId(createIllustrationDto.articleId), {'illustration': createdIllustration}).exec();
      } catch (e) {
  ...
  async update(id: string|ObjectId, updateIllustrationDto: UpdateIllustrationDto) {
    try {
      ...
      const updatedIllustration = await this.illustrationModel.findById(new ObjectId(id)).exec();
      // update user if avatar
      try {
  ...
  async remove(id: string|ObjectId) {
    try {
      ...
      try {
        ...
        throw new Error (`Avatar could not be removed from user with id ${deletedIllustration.userId} during illustration deletion: ${e}`);
      }
      // update article if illustration
      try {
        deletedIllustration.articleId && await this.articleModel.findByIdAndUpdate(deletedIllustration.userId, {'$delete': 'illustration'});
      } catch (e) {
        throw new Error (`Illustration could not be removed from user with id ${deletedIllustration.articleId} during illustration deletion: ${e}`);
      }
